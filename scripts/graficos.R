graficos = function(x, y, predicoes) {
  ...
  g = df %>% 
    ggplot(aes(x = x, y = y)) +
    geom_point() +
    geom_vline(xintercept = predicoes, color = "red", linetype = "dashed", show.legend = TRUE, linewidth = 0.5) +
    geom_vline(data = legend_data, aes(xintercept = intercept, color = color), linetype = "dashed", linewidth = 0.5, show.legend = TRUE) + 
